...
class Model(pl.LightningModule):
    ...
    def load_model(self, model_name: str, num_classes: int):
        """
        function assigns the specific model to use for training
        """
    ...
    def training_step(self, batch, batch_idx):
        """
        Defines a single step of training and logs training loss and accuracy.
        """
        data, targets = batch
        outputs = self(data)
    ...
    def test_step(self, batch, batch_idx):
        ...
        return {"test_loss": loss, "test_acc": self.accuracy, "test_precision": self.precision, "test_recall": self.recall, "test_f1": self.f1}
...
if __name__ == "__main__":
    # model names from timm: 'resnet50', "vgg16", "densenet121" or 'simple'
    model = Model(model_name="simple", num_classes=1, lr=1e-4, wd=1e-4)
